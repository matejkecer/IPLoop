...
void writeWLSForExactHighestPoleCoef_NParts(Diagram _diag,
	...
	for (int m = 0; m < N0; m++) {
		...
		s << _path << "Diagram_" << _diag.getName() << "_eps2_part_" << m
				<< ".wls";
...
void writeCombinedCVegasAndEx_3loop(Diagram _diag,
	...
	writeCVegas_eps1_3loop(_diag, _integVars, _poleCoefs, _overallFactor,
			_path);
	writeCVegas_eps0_3loop(_diag, _integVars, _poleCoefs, _overallFactor,
			_path);
...
void writeCombinedCVegasAndEx_2loop(Diagram _diag,
	...
	writeWLSForExactHighestPoleCoef_2loop(_diag, _integVars, _poleCoefs,
			_overallFactor, _path);
	writeCVegas_eps0_2loop(_diag, _integVars, _poleCoefs, _overallFactor,
			_path);
}
...
void writeCVegas_eps1_3loop(Diagram _diag,
		std::vector<GiNaC::symbol> _integVars,
		std::vector<GiNaC::ex> _poleCoefs, GiNaC::ex _overallFactor,
		std::string _path) {
...
void writeCVegas_eps0_3loop(Diagram _diag,
		std::vector<GiNaC::symbol> _integVars,
		std::vector<GiNaC::ex> _poleCoefs, GiNaC::ex _overallFactor,
		std::string _path) {
...
void writeCVegas_eps0_2loop(Diagram _diag,
		std::vector<GiNaC::symbol> _integVars,
		std::vector<GiNaC::ex> _poleCoefs, GiNaC::ex _overallFactor,
		std::string _path) {
...
//===========================================================================
// Sectorwise output
void writeCombinedCVegasAndEx_3loop_partwise(Diagram _diag,
		std::vector<GiNaC::symbol> _integVars,
		std::vector<GiNaC::ex> _poleCoefs, GiNaC::ex _overallFactor,
		std::string _path, int _k, std::string _cubaPath) {

	writeWLSForExactHighestPoleCoef_3loop_partwise(_diag, _integVars,
			_poleCoefs, _overallFactor, _path, _k);
	writeCVegas_eps1_3loop_partwise(_diag, _integVars, _poleCoefs,
			_overallFactor, _path, _k, _cubaPath);
	writeCVegas_eps0_3loop_partwise(_diag, _integVars, _poleCoefs,
			_overallFactor, _path, _k, _cubaPath);

}

void writeCombinedCVegasAndEx_2loop_partwise(Diagram _diag,
		std::vector<GiNaC::symbol> _integVars,
		std::vector<GiNaC::ex> _poleCoefs, GiNaC::ex _overallFactor,
		std::string _path, int _k, std::string _cubaPath) {

	writeWLSForExactHighestPoleCoef_2loop_partwise(_diag, _integVars,
			_poleCoefs, _overallFactor, _path, _k);
	writeCVegas_eps0_2loop_partwise(_diag, _integVars, _poleCoefs,
			_overallFactor, _path, _k, _cubaPath);

}

void writeCVegas_eps1_3loop_partwise(Diagram _diag,
		std::vector<GiNaC::symbol> _integVars,
		std::vector<GiNaC::ex> _poleCoefs, GiNaC::ex _overallFactor,
		std::string _path, int _k, std::string _cubaPath) {
	...
	std::stringstream s;
	s << _path << "eps1/" << "Diagram_" << _diag.getName() << "_eps1_part_"
			<< _k << ".cpp";
	...
	writer
			<< "int main(int argc, char *argv[]) { \n"
					"if (argc != 2) { \n"
					"		MAXEVAL = MAXEVAL_MANUAL; \n"
					"	}else{ \n"
					"		MAXEVAL = std::stod(argv[1]); \n"
					"	} \n"
					"\n"
					"clock_t begin = clock(); \n"
					"int comp, nregions, neval, fail; cubareal integral[NCOMP], error[NCOMP], prob[NCOMP]; \n"
					...
					"std::stringstream s; \n"
					"std::fstream writer;\n";
	writer << "s<<\"" << _path << "eps1/" << "Diagram_" << _diag.getName()
			<< "_eps1_part_" << _k << ".out\"; \n";
	writer << "writer.open(s.str(), std::fstream::out); \n"
			"writer<<std::fixed; \n"
			"writer.precision(10); \n";
...
void writeCVegas_eps0_3loop_partwise(Diagram _diag,
		std::vector<GiNaC::symbol> _integVars,
		std::vector<GiNaC::ex> _poleCoefs, GiNaC::ex _overallFactor,
		std::string _path, int _k, std::string _cubaPath) {
	...
	std::stringstream s;
	s << _path << "eps0/" << "Diagram_" << _diag.getName() << "_eps0_part_"
			<< _k << ".cpp";
	...
	writer
			<< "int main(int argc, char *argv[]) { \n"
					...
					"std::stringstream s; \n"
					"std::fstream writer;\n";
	writer << "s<<\"" << _path << "eps0/" << "Diagram_" << _diag.getName()
			<< "_eps0_part_" << _k << ".out\"; \n";
	writer << "writer.open(s.str(), std::fstream::out); \n"
			"writer<<std::fixed; \n"
			"writer.precision(10); \n";
...
void writeCVegas_eps0_2loop_partwise(Diagram _diag,
		std::vector<GiNaC::symbol> _integVars,
		std::vector<GiNaC::ex> _poleCoefs, GiNaC::ex _overallFactor,
		std::string _path, int _k, std::string _cubaPath) {
	...
	std::stringstream s;
	s << _path << "eps0/Diagram_" << _diag.getName() << "_eps0_part_" << _k
			<< ".cpp";
...
void writeWLSForExactHighestPoleCoef_3loop_partwise(Diagram _diag,
		std::vector<GiNaC::symbol> _integVars,
		std::vector<GiNaC::ex> _poleCoefs, GiNaC::ex _overallFactor,
	...
	s << _path << "eps2/" << "Diagram_" << _diag.getName() << "_eps2_part_"
			<< _k << ".wls";
	...
	writer << "op = OpenWrite[\"" << _path << "eps2/" << "Diagram_"
			<< _diag.getName() << "_eps2_part_" << _k << ".out\"]" << '\n';
	writer << "Write[op, overallFactor]" << '\n';
	//writer << "WriteString[op, \"exact coef. of pole eps^(" << currentPowerOfEps << "): \"]"
	//				<< '\n';
	writer << "Write[op, coef" << -currentPowerOfEps << "]" << '\n';
...
void writeWLSForExactHighestPoleCoef_2loop_partwise(Diagram _diag,
		std::vector<GiNaC::symbol> _integVars,
		std::vector<GiNaC::ex> _poleCoefs, GiNaC::ex _overallFactor,
		std::string _path, int _k) {
	...
	writer << "op = OpenWrite[\"" << _path << "eps1/" << "Diagram_"
			<< _diag.getName() << "_eps1_part_" << _k << ".out\"]" << '\n';
	writer << "Write[op, overallFactor]" << '\n';
	//writer << "WriteString[op, \"exact coef. of pole eps^(" << currentPowerOfEps << "): \"]"
	//				<< '\n';
	writer << "Write[op, coef" << -currentPowerOfEps << "]" << '\n';
...
void writeRunScript_2loop_partwise(Diagram _diag, std::string _path,
		int _numOfParts, std::string _cubaPath) {
	std::stringstream s1;
	s1 << _path << "run_eps1.sh";

	std::fstream writer;
	writer.open(s1.str(), std::fstream::out);
	writer << "#!/bin/bash \n";
	writer << "str=\"" << _diag.getName() << "\"\n \n"
			"cd eps1 \n \n"
			"for i in $(seq 0 " << _numOfParts - 1 << "); do \n"
			"\t chmod u+x \"Diagram_\"$str\"_eps1_part_\"$i\".wls\" \n"
			"\t ./\"Diagram_\"$str\"_eps1_part_\"$i\".wls\" \n"
			"done \n \n"
			"\cd .. \n";
	...
	std::stringstream s0;
	s0 << _path << "run_eps0.sh";
	writer.open(s0.str(), std::fstream::out);
	writer << "#!/bin/bash \n \n";
	writer << "str=\"" << _diag.getName() << "\"\n"
			"MAXEVAL=\"\" \n \n"
			"cd eps0 \n \n"
			"for i in $(seq 0 " << _numOfParts - 1
			<< "); do \n"
					"\t g++ -o eps0_$i \"Diagram_\"$str\"_eps0_part_\"$i\".cpp\" -lcuba -L\""
			<< _cubaPath << "\"\n"
					"\t ./eps0_$i $MAXEVAL\n"
					"done \n \n"
					"\cd .. \n";
...
void writeRunScript_3loop_partwise(Diagram _diag, std::string _path,
		int _numOfParts, std::string _cubaPath) {
	std::stringstream s2;
	s2 << _path << "run_eps2.sh";

	std::fstream writer;
	writer.open(s2.str(), std::fstream::out);
	writer << "#!/bin/bash \n";
	writer << "str=\"" << _diag.getName() << "\"\n \n"
			"cd eps2 \n \n"
			"for i in $(seq 0 " << _numOfParts - 1 << "); do \n"
			"\t chmod u+x \"Diagram_\"$str\"_eps2_part_\"$i\".wls\" \n"
			"\t ./\"Diagram_\"$str\"_eps2_part_\"$i\".wls\" \n"
			"done \n \n"
			"\cd .. \n";
	...
	writer << "str=\"" << _diag.getName() << "\"\n"
			...
			"for i in $(seq 0 " << _numOfParts - 1
			<< "); do \n"
					"\t g++ -o eps1_$i \"Diagram_\"$str\"_eps1_part_\"$i\".cpp\" -lcuba -L\""
			<< _cubaPath << "\"\n"
					"\t ./eps1_$i $MAXEVAL\n"
					"done \n \n"
					"\cd .. \n";
...
//===========================================================================
// GPU sectorwise output

void writeCombinedGPUVegasAndEx_3loop_partwise(Diagram _diag,
		std::vector<GiNaC::symbol> _integVars,
		std::vector<GiNaC::ex> _poleCoefs, GiNaC::ex _overallFactor,
		std::string _path, int _k) {
	...
	//writeWLSForExactHighestPoleCoef_3loop_partwise(_diag, _integVars,
	//		_poleCoefs, _overallFactor, _path, _k);
	writeGPUVegas_highestPoleCoef_3loop_partwise(_diag, _integVars, _poleCoefs,
			_overallFactor, _path, _k);
	writeGPUVegas_eps1_3loop_partwise(_diag, _integVars, _poleCoefs,
			_overallFactor, _path, _k);
	writeGPUVegas_eps0_3loop_partwise(_diag, _integVars, _poleCoefs,
			_overallFactor, _path, _k);

}

void writeCombinedGPUVegasAndEx_2loop_partwise(Diagram _diag,
		std::vector<GiNaC::symbol> _integVars,
		std::vector<GiNaC::ex> _poleCoefs, GiNaC::ex _overallFactor,
		std::string _path, int _k) {
	...
	writeGPUVegas_highestPoleCoef_2loop_partwise(_diag, _integVars, _poleCoefs,
			_overallFactor, _path, _k);
	writeGPUVegas_eps0_2loop_partwise(_diag, _integVars, _poleCoefs,
			_overallFactor, _path, _k);

}

void writeGPUVegas_highestPoleCoef_3loop_partwise(Diagram _diag,
		std::vector<GiNaC::symbol> _integVars,
		std::vector<GiNaC::ex> _poleCoefs, GiNaC::ex _overallFactor,
		std::string _path, int _k) {
	...
	std::string epsilonDir = "eps2";

	int currentPowerOfEps;

	std::fstream writer;

	_poleCoefs.at(0).print(GiNaC::print_csrc(aux));
	poleCoef = aux.str();

	std::stringstream s;
	s << _path << epsilonDir << "/Diagram_" << _diag.getName() << "_"
			<< epsilonDir << "_part_" << _k << ".cuh";

	writer.open(s.str(), std::fstream::out);

	writer << "#ifndef CONFIG_CUH\n"
			"#define CONFIG_CUH\n"
			"#include \"commons/functions.cuh\"\n"
			"#define _USE_MATH_DEFINES\n"
			"#include <math.h>\n\n";

	writer << "//vegas settings\n"
	...
	writer << "//integrand\n"
			"const int n_dim = " << _integVars.size() << ";\n\n";

	writer << "__host__ __device__ double integrand(double* xx) {\n\n";

	for (int j = 0; j < _integVars.size(); j++) {
		writer << "double " << _integVars.at(j) << " = xx[" << j << "];\n";
	}
	...
	writer << "std::string PATH = \"" << _path << epsilonDir << "/Diagram_"
			<< _diag.getName() << "_" << epsilonDir << "_part_" << _k
			<< ".out\"; \n"
					"std::string PREFACTOR = " << overallFactor << ";\n\n"
					"#endif";
	//-------------------------------------------------------------------
...
void writeGPUVegas_highestPoleCoef_2loop_partwise(Diagram _diag,
		std::vector<GiNaC::symbol> _integVars,
		std::vector<GiNaC::ex> _poleCoefs, GiNaC::ex _overallFactor,
		std::string _path, int _k) {
...
void writeGPUVegas_eps1_3loop_partwise(Diagram _diag,
		std::vector<GiNaC::symbol> _integVars,
		std::vector<GiNaC::ex> _poleCoefs, GiNaC::ex _overallFactor,
		std::string _path, int _k) {
...
void writeGPUVegas_eps0_3loop_partwise(Diagram _diag,
		std::vector<GiNaC::symbol> _integVars,
		std::vector<GiNaC::ex> _poleCoefs, GiNaC::ex _overallFactor,
		std::string _path, int _k) {
...
void writeGPUVegas_eps0_2loop_partwise(Diagram _diag,
		std::vector<GiNaC::symbol> _integVars,
		std::vector<GiNaC::ex> _poleCoefs, GiNaC::ex _overallFactor,
		std::string _path, int _k) {
...
void writeCompileScript_2loop_partwise_GPU(Diagram _diag, std::string _path,
		int _numOfParts) {

	std::fstream writer;

	std::stringstream s0;
	s0 << _path << "compile_eps0.sh";
	writer.open(s0.str(), std::fstream::out);
	writer << "#!/bin/bash \n \n";
	writer << "str=\"" << _diag.getName() << "\"\n \n"
	...
	writer << "rm functions.o \n"
			"cd .. \n";

	writer.close();

	std::stringstream s1;
	s1 << _path << "compile_eps1.sh";
	writer.open(s1.str(), std::fstream::out);
	writer << "#!/bin/bash \n \n";
	writer << "str=\"" << _diag.getName() << "\"\n \n"
			"XFLAGS=\"-fopenmp -DUSE_NVTX\" \n"
	...
	writer << "cd eps1 \n \n"
			...
			"for i in $(seq 0 " << _numOfParts - 1
			<< "); do \n"
					"\t filename=\"Diagram_\"$str\"_eps1_part_\"$i\".cuh\" \n"
					"\t mv $filename config.cuh \n"
					"\t nvcc -Xcompiler=$XFLAGS $NVCCFLAGS functions.o main.cu -o eps1_$i \n"
					"\t mv config.cuh $filename \n"
					"done \n \n";
...
void writeCompileScript_3loop_partwise_GPU(Diagram _diag, std::string _path,
		int _numOfParts) {
	...
	std::stringstream s2;
		...
		writer << "cd eps2 \n \n"
				...
				"for i in $(seq 0 " << _numOfParts - 1
				<< "); do \n"
						"\t filename=\"Diagram_\"$str\"_eps2_part_\"$i\".cuh\" \n"
						"\t mv $filename config.cuh \n"
						"\t nvcc -Xcompiler=$XFLAGS $NVCCFLAGS functions.o main.cu -o eps1_$i \n"
						"\t mv config.cuh $filename \n"
						"done \n \n";
...
void writeRunScript_2loop_partwise_GPU(Diagram _diag, std::string _path,
		int _numOfParts) {

	std::stringstream s1ex;
	s1ex << _path << "run_eps1_exact.sh";

	std::fstream writer;
	writer.open(s1ex.str(), std::fstream::out);
	writer << "#!/bin/bash \n";
	writer << "str=\"" << _diag.getName() << "\"\n \n"
			"cd eps1 \n \n"
			"for i in $(seq 0 " << _numOfParts - 1 << "); do \n"
...
void writeRunScript_3loop_partwise_GPU(Diagram _diag, std::string _path,
		int _numOfParts) {

	std::fstream writer;

	std::stringstream s2ex;
	s2ex << _path << "run_eps2_exact.sh";
	writer.open(s2ex.str(), std::fstream::out);
	writer << "#!/bin/bash \n";
	writer << "str=\"" << _diag.getName() << "\"\n \n"
			"cd eps2 \n \n"
			"for i in $(seq 0 " << _numOfParts - 1 << "); do \n"
